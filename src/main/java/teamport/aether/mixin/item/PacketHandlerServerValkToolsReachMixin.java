...
public abstract class PacketHandlerServerValkToolsReachMixin {
    ...
    private double fixReachDistanceForBlocks(double playerDist) {
        if (AetherToolMaterial.isHoldingValkyrieTool(playerEntity)) {
            ...
            if (playerDist < actualMaxDist) return 0;
        }
        return playerDist;
    }
    ...
    private double fixReachDistanceForEntities(PlayerServer instance, Entity entity, @NonNull Operation<Double> original) {
