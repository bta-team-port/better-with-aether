...
public class EntityRendererNeedle extends EntityRenderer<ProjectileNeedle> {
    ...
    @Override
    public void render(@NonNull TessellatorGeneral tessellator, @NonNull ProjectileNeedle needle, double x, double y, double z, float yaw, float partialTick) {
        this.bindTexture("/assets/aether/textures/entity/needle.png");
        GLRenderer.pushFrame();
        GLRenderer.modelM4f().translate((float) x, (float) y, (float) z);
        GLRenderer.modelM4f().rotate((float) Math.toRadians(needle.yRotO + (needle.yRot - needle.yRotO) * partialTick - 90.0F), 0.0F, 1.0F, 0.0F);
        GLRenderer.modelM4f().rotate((float) Math.toRadians(needle.xRotO + (needle.xRot - needle.xRotO) * partialTick), 0.0F, 0.0F, 1.0F);
        float bodyMinU = 0.0F;
        float bodyMaxU = 0.5F;
