...
public class MobRendererPhyg extends MobRendererQuadruped<MobPhyg> {
    ...
    @Override
    protected void renderSpecials(@NonNull TessellatorGeneral tessellator, @NonNull MobPhyg entity, double x, double y, double z) {
    ...
    @Override
    protected float getRenderAlpha(@NonNull MobPhyg entity, float partialTick) {
    ...
    @Override
    public float getShadowSize(@NonNull MobPhyg entity) {
        return entity.lookFadeAlpha < 0.9F ? 0.0F : super.getShadowSize(entity);
    }

    private boolean isRiderLookingAtOwnPig(@NonNull MobPhyg entity, float partialTick) {
    ...
    protected @Nullable StaticEntityModel getAndSetupModelForLayer(@NonNull MobPhyg entity, float brightness, float partialTick, int layer) {
        ...
        if (sitting && entity.onGround) {
            body.rotX = (double)-27.5F * (double)MathHelper.DEG_TO_RAD;
            body.posY = -4.0F;
            body.posZ = 1.0F;

            wingLeftInner.posY = -1.0F;
            wingLeftInner.rotX = -0.5F;
            wingRightInner.posY = -1.0F;
            wingRightInner.rotX = -0.5F;
