...
public class PlayerUntil {
    ...
    public static void damageItem(Player player, ItemStack stack, int itemDamage, InventoryType type, int index){
        ...
        if(stack.stackSize <= 0){
            switch (type){
                case HOLD:
                    player.destroyCurrentEquippedItem(); return;
                case MAIN:
                    player.inventory.mainInventory[index] = null; return;
                case ARMOR:
                    player.inventory.armorInventory[index] = null; return;
                default:
                    --stack.stackSize;
                    return;
            }
    ...
    /// The normal damageItem does not destroy the item if the item durability hits zero.
    /// This method is the simplest but relays on a the fact that negative itemstacks are deleted by default.
     public static void damageItem(Player player, int itemDamage, ItemStack stack){
        stack.damageItem(itemDamage, player);
        if(stack.stackSize <= 0){
            --stack.stackSize;
        }
    ...
    ///  Tool taking only 1 damage is quite common.
    public static void damageItem(Player player, ItemStack stack){
        damageItem(player, 1, stack);
    }
}
