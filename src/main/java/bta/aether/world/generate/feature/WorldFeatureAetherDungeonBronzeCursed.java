...
//        this.addSquareTube(world, random, x2 - 5, y2, z2 + 3, 6, 6, 6, 0);
    ...
    public boolean generateNextRoom(final World world, final Random random, final int i, final int j, final int k) {
        ...
        world.setBlock(x - 5, y, z + 3, Block.blockDiamond.id);
        world.setBlock(x + 3, y, z - 5, Block.blockDiamond.id);
        world.setBlock(x + 11, y, z + 3, Block.blockDiamond.id);
        world.setBlock(x + 3, y, z + 11, Block.blockDiamond.id);
    ...
    public void endCorridor(final World world, final Random random, final int i, final int j, final int k) {
        ...
        if (dir == 0) {
            ...
            z += 3;
            while (tunnelling) {
                if (this.isBoxSolid(world, x, y, z, 1, 8, 6) || x - i > 100) {
                    tunnelling = false;
                }
                boolean flag = true;
...
//                this.addPlaneX(world, random, x, y + 1, z + 1, 6, 4);
        ...
        if (dir == 1) {
            ...
            z += 11;
            while (tunnelling) {
                if (this.isBoxSolid(world, x, y, z, 6, 8, 1) || z - k > 100) {
                    tunnelling = false;
                }
                boolean flag = true;
                while (flag && (world.getBlockId(x, y, z) == AetherBlocks.stoneCarvedLocked.id || world.getBlockId(x, y, z) == AetherBlocks.stoneCarvedLightLocked.id || world.getBlockId(x, y, z) == AetherBlocks.stoneCarvedLight.id || world.getBlockId(x, y, z) == AetherBlocks.stoneCarved.id)) {
...
//                this.addPlaneZ(world, random, x + 1, y + 1, z, 4, 6);
        ...
        if (dir == 2) {
            ...
            z += 0;
            while (tunnelling) {
                if (this.isBoxSolid(world, x, y, z, 6, 8, 1) || j - z > 100) {
                    tunnelling = false;
                }
                boolean flag = true;
                while (flag && (world.getBlockId(x, y, z) == AetherBlocks.stoneCarvedLocked.id || world.getBlockId(x, y, z) == AetherBlocks.stoneCarvedLightLocked.id || world.getBlockId(x, y, z) == AetherBlocks.stoneCarvedLight.id || world.getBlockId(x, y, z) == AetherBlocks.stoneCarved.id)) {
